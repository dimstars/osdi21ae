...
static uint64_t getNewLogAllocator() {
    if (log_alloc_pool[my_thread_id % NUMACOUNT] == nullptr || node_alloc[my_thread_id % NUMACOUNT] == nullptr) {
        return -1;
    }
    LogAllocator* ret = new LogAllocator(log_alloc_pool[0]);
    return node_alloc[my_thread_id % NUMACOUNT]->getOff((char *)ret);
}
