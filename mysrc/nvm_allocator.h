...
static uint64_t getNewLogAllocator(int32_t numa_node) {
    if (numa_node < 0 || numa_node >= NUMACOUNT) {
        return -1;
    }
    if (log_alloc_pool[numa_node] == nullptr || node_alloc[numa_node] == nullptr) {
        return -1;
    }
    LogAllocator* ret = new LogAllocator(log_alloc_pool[numa_node]);
    return node_alloc[numa_node]->getOff((char *)ret);
}
