...
bool nvmpage::remove(btree *bt, entry_key_t key, bool only_rebalance,
  ...
  if (!only_rebalance) {
    ...
    // Remove the key from this node
    bool ret = remove_key(bt->pop[sub_root->nvm_numa], key);
    ...
    if (num_entries < left_num_entries) { // left -> right
      if (hdr.leftmost_ptr == nullptr) {
        for (int i = left_num_entries - 1; i >= m; i--) {
           insert_key(bt->pop[sub_root->nvm_numa], left_sibling->records[i].key,
                      left_sibling->records[i].ptr, &num_entries);
        }

        left_sibling->records[m].ptr = nullptr;
        pmemobj_persist(bt->pop[sub_root->nvm_numa], &(left_sibling->records[m].ptr),
                        sizeof(char *));

        left_sibling->hdr.last_index = m - 1;
        pmemobj_persist(bt->pop[sub_root->nvm_numa], &(left_sibling->hdr.last_index), 
                        sizeof(int16_t));

        parent_key = records[0].key;
      } else {
        insert_key(bt->pop[sub_root->nvm_numa], deleted_key_from_parent, (char *)hdr.leftmost_ptr,
                    &num_entries);
        ...
        pmemobj_persist(bt->pop[sub_root->nvm_numa], &(hdr.leftmost_ptr), sizeof(nvmpage *));
      ...
      hdr.is_deleted = 1;
      pmemobj_persist(bt->pop[sub_root->nvm_numa], &(hdr.is_deleted), sizeof(uint8_t));

      TOID(nvmpage) new_sibling;
      POBJ_NEW(bt->pop[sub_root->nvm_numa], &new_sibling, nvmpage, NULL, NULL);
      ...
      if (hdr.leftmost_ptr == nullptr) {
        for (int i = 0; i < num_dist_entries; i++) {
          left_sibling->insert_key(bt->pop[sub_root->nvm_numa], records[i].key, records[i].ptr,
                            &left_num_entries);
        }

        for (int i = num_dist_entries; records[i].ptr != NULL; i++) {
          D_RW(new_sibling)
              ->insert_key(bt->pop[sub_root->nvm_numa], records[i].key, records[i].ptr,
                            &new_sibling_cnt, false);
        }

        pmemobj_persist(bt->pop[sub_root->nvm_numa], D_RW(new_sibling), sizeof(nvmpage));

        left_sibling->hdr.sibling_ptr = (nvmpage *)new_sibling.oid.off;
        pmemobj_persist(bt->pop[sub_root->nvm_numa], &(left_sibling->hdr.sibling_ptr),
                        sizeof(nvmpage *));

        parent_key = D_RO(new_sibling)->records[0].key;
      } else {
        left_sibling->insert_key(bt->pop[sub_root->nvm_numa], deleted_key_from_parent,
...
bool nvmpage::merge(btree *bt, bpnode *left_sibling, entry_key_t deleted_key_from_parent, subtree* sub_root, subtree* left_subtree_sibling) {
  ...
  if (total_num_entries > nvm_cardinality - 1) { // Redistribution
    ...
    if (num_entries < left_num_entries) { // left -> right
      ...
      // pmemobj_persist(bt->pop[sub_root->nvm_numa], sub_root, sizeof(subtree));
...
void nvmpage::insert_key(PMEMobjpool *pop, entry_key_t key, char *ptr,
...
nvmpage *nvmpage::store(btree *bt, char *left, entry_key_t key, char *right, bool flush,
  ...
  if (num_entries < nvm_cardinality - 1) {
    insert_key(bt->pop[sub_root->nvm_numa], key, right, &num_entries, flush);
    return (nvmpage *)pmemobj_oid(this).off;
  } else { // FAIR
    // overflow
    // create a new node
    TOID(nvmpage) sibling;
    POBJ_NEW(bt->pop[sub_root->nvm_numa], &sibling, nvmpage, NULL, NULL);
    ...
    if (hdr.leftmost_ptr == NULL) { // leaf node
      for (int i = m; i < num_entries; ++i) {
        sibling_ptr->insert_key(bt->pop[sub_root->nvm_numa], records[i].key, records[i].ptr,
                                &sibling_cnt, false);
      }
    } else { // internal node
    ...
    pmemobj_persist(bt->pop[sub_root->nvm_numa], sibling_ptr, sizeof(nvmpage));

    hdr.sibling_ptr = (nvmpage *)sibling.oid.off;
    pmemobj_persist(bt->pop[sub_root->nvm_numa], &hdr, sizeof(hdr));
    ...
    if (IS_FORWARD(hdr.switch_counter))
      ...
      ++hdr.switch_counter;
    records[m].ptr = NULL;
    pmemobj_persist(bt->pop[sub_root->nvm_numa], &records[m], sizeof(nvmentry));

    hdr.last_index = m - 1;
    pmemobj_persist(bt->pop[sub_root->nvm_numa], &hdr.last_index, sizeof(int16_t));
    ...
    // insert the key
    if (key < split_key) {
      insert_key(bt->pop[sub_root->nvm_numa], key, right, &num_entries);
      ret = (nvmpage *)pmemobj_oid(this).off;
    } else {
      sibling_ptr->insert_key(bt->pop[sub_root->nvm_numa], key, right, &sibling_cnt);
    ...
    if (sub_root != NULL && hdr.level == nvm_root->hdr.level) { // subtree root
      subtree* next = newSubtreeRoot(bt->pop, sub_root->nvm_numa, (nvmpage *)sibling.oid.off, sub_root);
...
void subtree::subtree_update(btree* root, entry_key_t key, char* right, bool wal) {
  ...
  if (flag) {
    ...
    if (!p->update_key(pop[nvm_numa], key, right)) { // store
      // printf("no such key\n");
    }
  }
  subtree_unlock();
...
void subtree::nvm_to_dram(bpnode **pre) {
  ...
  // nvm_ptr = nullptr;
  log_off = getNewLogAllocator(nvm_numa);
  log_alloc = node_alloc[nvm_numa]->getNVMptr(log_off);
}
...
char* subtree::DFS(char* root, nvmpage **pre, bool ifdel) {
    ...
    if (node->hdr.nvmpage_off != -1) { // 复用nvmpage
      ...
      POBJ_NEW(pop[nvm_numa], &nvm_node, nvmpage, NULL, NULL);
    ...
    if (isflush) {
      //TX_BEGIN(pop) {
        ...
        pmemobj_memcpy_persist(pop[nvm_numa], nvm_node_ptr, tmp_ptr, sizeof(nvmpage));
      //} TX_END
    }
...
void subtree::subtree_search_range(entry_key_t min, entry_key_t max, std::vector<std::pair<uint64_t, uint64_t>>& results, int &size) {
  if (flag) {
    ...
    p->linear_search_range(min, max, results, size, (uint64_t)pop[nvm_numa]);
  } else {
    nvmpage* p = get_nvmroot_ptr();
    while (p->hdr.leftmost_ptr != NULL) {
...
void subtree::subtree_search_range(entry_key_t min, entry_key_t max, std::vector<std::string> &values, int &size) {
  if (flag) {
    ...
    p->linear_search_range(min, max, values, size, (uint64_t)pop[nvm_numa]);
  } else {
    nvmpage* p = get_nvmroot_ptr();
    while (p->hdr.leftmost_ptr != NULL) {
      p = to_nvmpage(p->linear_search(min));
...
void subtree::recover() {
  ...
  // 恢复出原来的日志
  LogAllocator* old_log_alloc = node_alloc[nvm_numa]->getNVMptr(log_off);
  old_log_alloc->recovery(log_alloc_pool[nvm_numa]);
  ...
  for (int i = 0; (tmp = old_log_alloc->getNextLogNode(i)) != nullptr; i++) {
    ...
    case 1:
      {// insert todo：存在就不insert
        ...
        p->insert_key(pop[nvm_numa], tmp->key, (char *)tmp->value, &num_entries,true);
        break;
      }
    case 2:
      {// update
        p->update_key(pop[nvm_numa], tmp->key, (char *)tmp->value);
        break;
      }
    case 0:
      {// delete
        p->remove_key(pop[nvm_numa], tmp->key);
        break;
      }
    case 3:
      {
        ...
        if(p->hdr.leftmost_ptr == NULL){ // leaf node
          for(int i=m; i<num_entries; ++i){ 
            dst->insert_key(pop[nvm_numa], p->records[i].key, p->records[i].ptr, &sibling_cnt, false);
          }
        }
        else{ // internal node
          for(int i=m+1;i<num_entries;++i){ 
        ...
        pmemobj_persist(pop[nvm_numa], dst, sizeof(nvmpage));

        p->hdr.sibling_ptr = (nvmpage *)pmemobj_oid(dst).off;
        pmemobj_persist(pop[nvm_numa], &(p->hdr), sizeof(p->hdr));
        ...
        pmemobj_persist(pop[nvm_numa], &(p->records[m]), sizeof(nvmentry));

        p->hdr.last_index = m - 1;
        pmemobj_persist(pop[nvm_numa], &(p->hdr.last_index), sizeof(int16_t));
      ...
      {// 子树内合并 dram -- dram
        ...
        if (total_num_entries > nvm_cardinality - 1) { //Redistribution
          ...
          if (num_entries < left_num_entries) { // left -> right
            ...
            if (cur->hdr.leftmost_ptr == nullptr) {
              for (int i = left_num_entries - 1; i >= m; i--) {
                cur->insert_key(pop[nvm_numa], p->records[i].key,
                            p->records[i].ptr, &num_entries);
              }

              p->records[m].ptr = nullptr;
              pmemobj_persist(pop[nvm_numa], &(p->records[m].ptr),
                              sizeof(char *));

              p->hdr.last_index = m - 1;
              pmemobj_persist(pop[nvm_numa], &(p->hdr.last_index), 
                              sizeof(int16_t));
            } else {
              cur->insert_key(pop[nvm_numa], deleted_key_from_parent, (char *)cur->hdr.leftmost_ptr,
                          &num_entries);
              ...
              pmemobj_persist(pop[nvm_numa], &(cur->hdr.leftmost_ptr), sizeof(nvmpage *));
            ...
            if (cur->hdr.leftmost_ptr == nullptr) {
              for (int i = 0; i < num_dist_entries; i++) {
                p->insert_key(pop[nvm_numa], cur->records[i].key, cur->records[i].ptr,
                                  &left_num_entries);
              }

              for (int i = num_dist_entries; node_tmp->records[i].ptr != NULL; i++) {
                    cur->insert_key(pop[nvm_numa], node_tmp->records[i].key, node_tmp->records[i].ptr,
                                  &new_sibling_cnt, false);
              }

              pmemobj_persist(pop[nvm_numa], cur, sizeof(nvmpage));

              p->hdr.sibling_ptr = (nvmpage *)pmemobj_oid(cur).off;
              pmemobj_persist(pop[nvm_numa], &(p->hdr.sibling_ptr),
                              sizeof(nvmpage *));
            } else {
              p->insert_key(pop[nvm_numa], deleted_key_from_parent,
          ...
          pmemobj_persist(pop[nvm_numa], &(cur->hdr.is_deleted), sizeof(uint8_t));
...
void subtree::recovery(btree* bt) {
  ...
  for(int i = 0; i < NUMACOUNT; ++i) {
    log_alloc = node_alloc[i]->getNVMptr(log_off);
    log_alloc->recovery(log_alloc_pool[i]);
  }
...
void MyBtree::constructor(PMEMobjpool * pool[]) {
  ...
  pmemobj_persist(pop[0], this, sizeof(MyBtree));
}
...
void MyBtree::Recover(PMEMobjpool *pool[]) {
  ...
  if (head == nullptr) { // 没有进行转换 读log恢复
    ...
    bt = new btree(pop, 4);
    bt->setFlag2(true);
    //ptr->pop = pool[my_thread_id % NUMACOUNT];
    //ptr->recover();
    ptr->recovery(bt);
