...
bool nvmpage::remove(btree *bt, entry_key_t key, bool only_rebalance,
  ...
  if (!only_rebalance) {
    ...
    // Remove the key from this node
    bool ret = remove_key(bt->pop[my_thread_id % NUMACOUNT], key);
    ...
    if (num_entries < left_num_entries) { // left -> right
      if (hdr.leftmost_ptr == nullptr) {
        for (int i = left_num_entries - 1; i >= m; i--) {
           insert_key(bt->pop[my_thread_id % NUMACOUNT], left_sibling->records[i].key,
                      left_sibling->records[i].ptr, &num_entries);
        }

        left_sibling->records[m].ptr = nullptr;
        pmemobj_persist(bt->pop[my_thread_id % NUMACOUNT], &(left_sibling->records[m].ptr),
                        sizeof(char *));

        left_sibling->hdr.last_index = m - 1;
        pmemobj_persist(bt->pop[my_thread_id % NUMACOUNT], &(left_sibling->hdr.last_index), 
                        sizeof(int16_t));

        parent_key = records[0].key;
      } else {
        insert_key(bt->pop[my_thread_id % NUMACOUNT], deleted_key_from_parent, (char *)hdr.leftmost_ptr,
                    &num_entries);
        ...
        pmemobj_persist(bt->pop[my_thread_id % NUMACOUNT], &(hdr.leftmost_ptr), sizeof(nvmpage *));
      ...
      hdr.is_deleted = 1;
      pmemobj_persist(bt->pop[my_thread_id % NUMACOUNT], &(hdr.is_deleted), sizeof(uint8_t));

      TOID(nvmpage) new_sibling;
      POBJ_NEW(bt->pop[my_thread_id % NUMACOUNT], &new_sibling, nvmpage, NULL, NULL);
      ...
      if (hdr.leftmost_ptr == nullptr) {
        for (int i = 0; i < num_dist_entries; i++) {
          left_sibling->insert_key(bt->pop[my_thread_id % NUMACOUNT], records[i].key, records[i].ptr,
                            &left_num_entries);
        }

        for (int i = num_dist_entries; records[i].ptr != NULL; i++) {
          D_RW(new_sibling)
              ->insert_key(bt->pop[my_thread_id % NUMACOUNT], records[i].key, records[i].ptr,
                            &new_sibling_cnt, false);
        }

        pmemobj_persist(bt->pop[my_thread_id % NUMACOUNT], D_RW(new_sibling), sizeof(nvmpage));

        left_sibling->hdr.sibling_ptr = (nvmpage *)new_sibling.oid.off;
        pmemobj_persist(bt->pop[my_thread_id % NUMACOUNT], &(left_sibling->hdr.sibling_ptr),
                        sizeof(nvmpage *));

        parent_key = D_RO(new_sibling)->records[0].key;
      } else {
        left_sibling->insert_key(bt->pop[my_thread_id % NUMACOUNT], deleted_key_from_parent,
...
bool nvmpage::merge(btree *bt, bpnode *left_sibling, entry_key_t deleted_key_from_parent, subtree* sub_root, subtree* left_subtree_sibling) {
  ...
  if (total_num_entries > nvm_cardinality - 1) { // Redistribution
    ...
    if (num_entries < left_num_entries) { // left -> right
      ...
      // pmemobj_persist(bt->pop[my_thread_id % NUMACOUNT], sub_root, sizeof(subtree));
...
nvmpage *nvmpage::store(btree *bt, char *left, entry_key_t key, char *right, bool flush,
  ...
  if (num_entries < nvm_cardinality - 1) {
    insert_key(bt->pop[my_thread_id % NUMACOUNT], key, right, &num_entries, flush);
    return (nvmpage *)pmemobj_oid(this).off;
  } else { // FAIR
    // overflow
    // create a new node
    TOID(nvmpage) sibling;
    POBJ_NEW(bt->pop[my_thread_id % NUMACOUNT], &sibling, nvmpage, NULL, NULL);
    ...
    if (hdr.leftmost_ptr == NULL) { // leaf node
      for (int i = m; i < num_entries; ++i) {
        sibling_ptr->insert_key(bt->pop[my_thread_id % NUMACOUNT], records[i].key, records[i].ptr,
                                &sibling_cnt, false);
      }
    } else { // internal node
    ...
    pmemobj_persist(bt->pop[my_thread_id % NUMACOUNT], sibling_ptr, sizeof(nvmpage));

    hdr.sibling_ptr = (nvmpage *)sibling.oid.off;
    pmemobj_persist(bt->pop[my_thread_id % NUMACOUNT], &hdr, sizeof(hdr));
    ...
    if (IS_FORWARD(hdr.switch_counter))
      ...
      ++hdr.switch_counter;
    records[m].ptr = NULL;
    pmemobj_persist(bt->pop[my_thread_id % NUMACOUNT], &records[m], sizeof(nvmentry));

    hdr.last_index = m - 1;
    pmemobj_persist(bt->pop[my_thread_id % NUMACOUNT], &hdr.last_index, sizeof(int16_t));
    ...
    // insert the key
    if (key < split_key) {
      insert_key(bt->pop[my_thread_id % NUMACOUNT], key, right, &num_entries);
      ret = (nvmpage *)pmemobj_oid(this).off;
    } else {
      sibling_ptr->insert_key(bt->pop[my_thread_id % NUMACOUNT], key, right, &sibling_cnt);
    ...
    if (sub_root != NULL && hdr.level == nvm_root->hdr.level) { // subtree root
      subtree* next = newSubtreeRoot(bt->pop[my_thread_id % NUMACOUNT], (nvmpage *)sibling.oid.off, sub_root);
...
void subtree::nvm_to_dram(bpnode **pre) {
  ...
  log_alloc = node_alloc[my_thread_id % 2]->getNVMptr(log_off);
}
...
void subtree::sync_subtree(nvmpage **pre) {
  ...
  if (log_alloc) log_alloc->DeleteLog();
  log_off = getNewLogAllocator();
  log_alloc = node_alloc[my_thread_id % NUMACOUNT]->getNVMptr(log_off);
}
...
void subtree::recover() {
  ...
  // 恢复出原来的日志
  LogAllocator* old_log_alloc = node_alloc[my_thread_id % NUMACOUNT]->getNVMptr(log_off);
  old_log_alloc->recovery(log_alloc_pool[my_thread_id % NUMACOUNT]);
...
void MyBtree::exitBtree() {
  ...
  delete bt;
  pmemobj_close(pop[my_thread_id % NUMACOUNT]);
}
