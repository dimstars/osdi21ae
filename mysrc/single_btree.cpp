...
btree::btree(PMEMobjpool *pool){
  ...
  for (int i=0; i<NUMACOUNT; ++i) {
    log_off = getNewLogAllocator(i);
    log_alloc[i] = node_alloc[i]->getNVMptr(log_off);
    log_alloc[i]->setCapacity(BigLogSize);
  }
  it = nullptr;
}
...
char *btree::findSubtreeRoot(entry_key_t key) {
    ...
    if (tmp->change != tmp->flag && tmp->lock == false) {
      if (tmp->getState() && my_thread_id % NUMACOUNT == tmp->hotspot_numa) { // 让热点numa节点上的线程执行缓存操作
        bpnode *pre = nullptr;
        if (tmp->getPrePtr() != nullptr) {
          pre = (bpnode *)tmp->getPrePtr()->getLastLeafNode();
        }
        tmp->nvm_to_dram(&pre);
      //} else if (!tmp->getState() && my_thread_id % NUMACOUNT == tmp->nvm_numa) { // 让原numa节点上的线程执行同步操作
...
/*
void btree::checkHeat() {
  if (!flag) return;
  
  uint64_t min_key = 0;
  bpnode* p = (bpnode *)root;
  while (p->hdr.level != tar_level && p->hdr.leftmost_ptr != nullptr) {
      p = (bpnode *)p->linear_search(min_key);
  }

  for(int i=0; p->records[i].ptr != NULL; ++i) {
    subtree *tmp = (subtree *)p->records[i].ptr;
    tmp->refreshHeat();
  }  
}
*/
...
void btree::deform() {
  ...
  {  // for debug
    ...
    while(q) {
        q->hdr.leftmost_ptr = (bpnode *)newSubtreeRoot(pop, my_thread_id % NUMACOUNT, q->hdr.leftmost_ptr);
        ...
        for (int i = 0; i <= q->hdr.last_index; i++) {
            q->records[i].ptr = (char *)newSubtreeRoot(pop, my_thread_id % NUMACOUNT, (bpnode *)q->records[i].ptr);
...
void btree::scan_all_leaf() {
    ...
    if (sub_root->flag) {
      ...
      leaf = (bpnode *)((uint64_t)leaf + (uint64_t)pop[sub_root->nvm_numa]);
      nvm_num++;
    }
    int totul_num = 0;
    while (leaf != nullptr) {
      ...
      if (IS_VALID_PTR(leaf->hdr.sibling_ptr)) {
        ...
        leaf = (bpnode *)((uint64_t)leaf->hdr.sibling_ptr + (uint64_t)pop[sub_root->nvm_numa]);
      } else {
        break;
      }
    }
...
bool bpnode::remove(btree* bt, entry_key_t key, bool only_rebalance, bool with_lock, subtree* sub_root) {
  if(!only_rebalance) {
    ...
    if(!should_rebalance) {
      ret = (hdr.leftmost_ptr == NULL) ? ret : true;
      unlock();
      return ret;
    }
    unlock();
  ...
  if(total_num_entries > cardinality - 1) { // Redistribution
    ...
    if(num_entries < left_num_entries) { // left -> right
...

      {
        ...
        for(int i=0; i<num_dist_entries - 1; i++){
          left_sibling->insert_key(bt->pop[sub_root->nvm_numa], records[i].key, sub_root->DFS(records[i].ptr, &pre),
              &left_num_entries); 
        } 
    ...
    for(int i = 0; records[i].ptr != NULL; ++i) { 
      left_sibling->insert_key(bt->pop[sub_root->nvm_numa], records[i].key, sub_root->DFS(records[i].ptr, &pre), &left_num_entries);
    }
...
bool bpnode::update_key(entry_key_t key, char* ptr) {
  register int num_entries = count();
...
// 采用FAST&FAIR方案进行插入操作
void bpnode::insert_key(entry_key_t key, char* ptr, int *num_entries) {
...
bpnode *bpnode::store(btree* bt, char* left, entry_key_t key, char* right,
  ...
  if(num_entries < cardinality - 1) {
    ...
    if (sub_root != NULL && hdr.level <= sub_root->dram_ptr->hdr.level) {
      ...
      POBJ_NEW(bt->pop[sub_root->nvm_numa], &nvm_node, nvmpage, NULL, NULL);
    ...
    if (sub_root != NULL && hdr.level == sub_root->dram_ptr->hdr.level) { // subtree root
      subtree* next = newSubtreeRoot(bt->pop, sub_root->nvm_numa, sibling, sub_root);
      // sync
      nvmpage *pre = nullptr;
...
char* btree::DFS(char* root) {
    ...
    TOID(nvmpage) nvm_node;
    POBJ_NEW(pop[my_thread_id % NUMACOUNT], &nvm_node, nvmpage, NULL, NULL);
    D_RW(nvm_node)->constructor();
    ...
    pmemobj_persist(pop[my_thread_id % NUMACOUNT], nvm_node_ptr, sizeof(nvmpage));
    delete node;
