  ...
  if(total_num_entries > cardinality - 1) { // Redistribution
    ...
    if(num_entries < left_num_entries) { // left -> right
...

      {
        ...
        for(int i=0; i<num_dist_entries - 1; i++){
          left_sibling->insert_key(bt->pop[my_thread_id % NUMACOUNT], records[i].key, sub_root->DFS(records[i].ptr, &pre),
              &left_num_entries); 
        } 
    ...
    for(int i = 0; records[i].ptr != NULL; ++i) { 
      left_sibling->insert_key(bt->pop[my_thread_id % NUMACOUNT], records[i].key, sub_root->DFS(records[i].ptr, &pre), &left_num_entries);
    }
...
bpnode *bpnode::store(btree* bt, char* left, entry_key_t key, char* right,
  ...
  if(num_entries < cardinality - 1) {
    ...
    if (sub_root != NULL && hdr.level <= sub_root->dram_ptr->hdr.level) {
      ...
      POBJ_NEW(bt->pop[my_thread_id % NUMACOUNT], &nvm_node, nvmpage, NULL, NULL);
    ...
    if (sub_root != NULL && hdr.level == sub_root->dram_ptr->hdr.level) { // subtree root
      subtree* next = newSubtreeRoot(bt->pop[my_thread_id % NUMACOUNT], sibling, sub_root);
      // sync
      nvmpage *pre = nullptr;
